package pers.xuliushen.expressstation_exception.view;

import pers.xuliushen.expressstation_exception.dao.ExpressDao;
...
import pers.xuliushen.expressstation_exception.exception.OutNumberBoundException;
...
public class Views {
    ...
    public void addExpress(){
        ...
        try {
            ...
            System.out.println("添加成功，放在了快递柜中的第"+coordinate.getX()+"排，第"+coordinate.getY()+"列");
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
    ...
    public void updateExpress(){
        ...
        if (expressDao.findByNumber(oldNumber) == null) {
            System.err.println("快递不存在，修改失败！");
        }else {
            //修改对象信息
...
//        }
    ...
    private void printAllExpresses(){
        Express[][] expresses = expressDao.findAll();
        int count = 0;
        for(int i=0;i<10;i++){
            for(int j=0;j<10;j++) {
     ...
     * @param number 快递单号
     */
    private void printAllByGrid(String number){
        Express[][] expresses = expressDao.findAll();
        for(int i=0;i<10;i++){
            for(int j=0;j<10;j++) {
                if(expresses[i][j] != null) {
                    System.out.printf("%s-30",expresses[i][j]);
                }else{
                    System.out.printf("%s-30","空");
                }
            }
            System.out.println();
        }
